...
;; predicate
(is-print (KORZ-TESTs (method () plus (a) (< a :rcvr 0))
		      (method () minus (a) (> a :rcvr 0))
		      (method (:rcvr @plus) hoge ()
			      (print "plus"))
		      (method (:rcvr @minus) hoge ()
			      (print "minus"))
		      (method () hoge ()
			      (print "zero"))
		      (hoge :rcvr -1))
...
(is-print (KORZ-TESTs (method () plus (a) (< a :rcvr 0))
		      ...
		      (method (:equal true) plus (a)
			      (<= a :rcvr 0))
		      (method (:equal @plus) plus (a)
			      (print "it is never printed"))
		      (method (:rcvr @plus) hoge ()
			      (print "plus"))
		      (method (:rcvr @minus) hoge ()
			      (print "minus"))
...
(is-error (KORZ-TESTs (method () plus (a) (< a :rcvr 0))
		      (method () minus (a) (> a :rcvr 0))
		      (method (:equal true) plus (a)
			      (<= a :rcvr 0))
		      (method (:equal true) minus (a)
			      (>= a :rcvr 0))
		      (method (:rcvr @plus) hoge ()
			      (print "plus"))
		      (method (:rcvr @minus) hoge ()
...
(is-print (korz-tests (def () pospare (newcoord))
		      (def () position (newcoord (pospare)))
		      (var (:rcvr (position)) x 0)
		      (method () plusp ((p (pospare)))
			      (> 0 :rcvr (x :rcvr p)))
		      (method () hoge()
			      (print "hoge"))
		      (method (:rcvr @plusp) hoge()
			      (print "plus hoge"))
		      (let ((p (copy (position))))
			(set (x :rcvr p) 1)
			(hoge :rcvr p)))
